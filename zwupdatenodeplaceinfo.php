    ...
    if (!is_empty($placeList)) {
            ...
            if ($initFlag == 1) {
                ...
                if ($hasMsg) {
                    ...
                    if (sqlsrv_has_rows($result)) {
                        $sql = "UPDATE AZW009_serialrelation SET enddate=CONVERT(VARCHAR(10),GETDATE(),120)
                    ...
                    if (!is_empty($serial) && !is_empty($customerId) && !is_empty($sensorId)) {
                        ...
                        VALUES('$serial','$sensorId','$facilityCd','$customerId','$displayCd','$place','$memo',CONVERT(VARCHAR(10),GETDATE()+1,120),1)";
        ...
        if ($code == '200' && (!is_empty($updateItemMsg) || !is_empty($insertItemMsg))) {
            ...
            $insertSql = "INSERT INTO AZW152_vznoticetbl(receiveuser,senduser,noticetype,title,registdate,content)
                          SELECT TOP 1 '$staffId','$customerId','S','センサーの設置情報が更新されました',CONVERT(VARCHAR(19),GETDATE(),120),
