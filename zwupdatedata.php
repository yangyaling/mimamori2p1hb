                    ...
                    if ($handle && !hasError($handle)) {
                        ...
                        if ($res) {
                            foreach ($res as $key => $node) {
                                ...
                                $sql = $iSql . "('$deviceId',$value,'$unit',$timeValue,'$strDate',$strHour,$strMin,0)";
                                $result = sqlsrv_query($conn, $sql);
                                if (!$result) {
                            ...
                            if ($lastTimestamp != 0) {
                                $sql = "UPDATE AZW137_zworksdevicemst
                    ...
                    if ($handle && !hasError($handle)) {
                        ...
                        if ($res) {
                            ...
                            foreach ($res as $key => $node) {
                                ...
                                $vSql .= "('$deviceId',$value,'$unit',$timeValue,'$strDate',$strHour,$strMin,0),";

                                $lastTimestamp = $timeValue;
                            }
                            if (is_empty($vSql)) {
                                $result = sqlsrv_query($conn, $iSql . substr($vSql, 0, strlen($vSql) - 1));
                                if (!$result) {
                                    $errors = sqlsrv_errors();
                                    $code = '504';
                                }
