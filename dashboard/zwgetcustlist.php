...
//    $staffId = 'sw00001';
    ...
    if ($facilityCd == DEFAULT_FACILITY_CD) {
        $sql = "SELECT fv.roomcd,fv.custid,fv.custname,fv.picpath,DATEDIFF(DAY,CAST(fv.birthday AS DATE)," . $SCH . ".GETJPDATE()) / 365 old,
                ...
                WHERE fv.hostcd='$hostCd' GROUP BY fv.facilitycd,fv.facilityname2,fv.floorno,fv.roomcd,fv.custid,fv.custname,fv.picpath,
                fv.birthday,fv.sex,cv.tvalue,cv.tunit,cv.bd ORDER BY fv.facilitycd,fv.floorno,fv.roomcd";
    } else {
        $sql = "SELECT fv.roomcd,fv.custid,fv.custname,fv.picpath,
            ...
            fv.picpath,fv.birthday,fv.sex,cv.tvalue,cv.tunit,cv.bd ORDER BY fv.roomcd";
    }
