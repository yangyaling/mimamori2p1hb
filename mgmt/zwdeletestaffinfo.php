...
if ($conn) {
    ...
    if ($code == '200') {
        $sql = "SELECT 1 FROM AZW007_staffrelation WHERE staffid='$staffId' AND facilitycd='$facilityCd'";

        if (sqlsrv_has_rows(sqlsrv_query($conn, $sql))) {
            $sql = "DELETE FROM AZW007_staffrelation WHERE staffid='$staffId' AND facilitycd='$facilityCd'";
...
//                      ('$staffId','$customerId','S','センサーの設置情報が更新されました',CONVERT(VARCHAR(19)," . $SCH . ".GETJPDATE(),120),'【$roomCd】　$customerNameさんの情報が削除されました。')";
//
